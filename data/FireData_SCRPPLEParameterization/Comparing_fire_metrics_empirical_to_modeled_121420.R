...
# landscape outline
outline <- readOGR("/Users/shelbyweiss/Documents/FireData_SCRPPLEParameterization/FullLandscape/StudyArea_proj.shp")
plot(outline)
# climate map to serve as reference raster map with correct resolution/extent
clim0 <- raster("/Users/shelbyweiss/Documents/FULL_Landscape_Inputs_AK_060921_COPYME/AK_ClimateMap_10_Regions.tif")
...
dalton_hist_over4 <- subset(dalton_hist_over4, dalton_hist_over4$FIREYEAR>=1984 & dalton_hist_over4$FIREYEAR<=1990)
#dalton_hist_over4 <- dalton_hist
...
timeperiod_hist <- max(as.numeric(paste(dalton_hist_over4$FIREYEAR))) - min(as.numeric(paste(dalton_hist_over4$FIREYEAR)))

dalton_hist_rast <- rasterize(dalton_hist_over4, clim0)
#plot(dalton_hist_rast)
totalarea <- 5242453*cells_per_ha     # this is the number of active cells, converted to hectares
...
burnedarea_hist <- sum(dalton_hist_df)*cells_per_ha
...
write.csv(dalton_hist_size_grouped, "C:/Users/13146/Dropbox/NSF ReburnsAK LANDIS Results/landscape/R/Fire_Stats/FULL_hist_size_grouped_AKLFDB.csv")
...
summary(as.numeric(fire_hist$FIREYEAR))
####SIMULATION RESLUTS####
