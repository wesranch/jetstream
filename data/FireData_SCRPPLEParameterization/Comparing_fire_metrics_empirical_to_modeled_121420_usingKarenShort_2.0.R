...
fire_hist <- readOGR("/Users/shelbyweiss/Documents/FireData_SCRPPLEParameterization/AKstudyarea_Fires_Lightning.shp")
#dalton_perim_latlong <- spTransform(dalton_perims, crs(fire_hist))
outline <- readOGR(paste0(dir, "StudyArea_11milha_072221.shp")) 
clim0_outlinelatlong <- spTransform(outline, crs(fire_hist))
...
dalton_hist <- crop(fire_hist, (outline))
...
dalton_hist_over4$FIRE_YEAR <- as.numeric(paste(dalton_hist_over4$FIRE_YEAR))
dalton_hist_over4 <- subset(dalton_hist_over4, dalton_hist_over4$FIRE_YEAR>=1992 & dalton_hist_over4$FIRE_YEAR<=2015)
summary((((dalton_hist_over4$FIRE_YEAR))))
###Historic fire rotation
####fire rotation (ie total time / proportion of area burned during that time)####
ha_per_cell <-4
#rasterize dalton_hist
timeperiod_hist <- max(as.numeric(paste(dalton_hist_over4$FIRE_YEAR))) - min(as.numeric(paste(dalton_hist_over4$FIRE_YEAR))) + 1
...
write.csv(dalton_hist_size_grouped, "/Users/shelbyweiss/Documents/FireData_SCRPPLEParameterization/FULL11milha_hist_size_grouped_KarenShort_1992_to_2015.csv")
