...
```{r, Plotting Climate Data}

#plotting landis climate
climateFuture <- read.csv("F:/Rancher_Sims_Full_Landscape/FinalSimulationsThesis/rep1_4.3.2025/interior_hist_ncar/Climate-future-monthly-input-log.csv")
climateFuture_temp <- climateFuture %>%
  select(Year, MinTemp, MaxTemp)%>%
  group_by(Year)%>%
  summarise(AvgTemp=(mean(MinTemp, na.rm = TRUE)+mean(MaxTemp, na.rm = TRUE) / 2),
            StDev=(sd(MinTemp, na.rm = TRUE)+sd(MaxTemp, na.rm = TRUE) / 2))
...
ggplot(aes(x = Year, y = AvgProbEst, color = EcoRegion)) +
  ...
  scale_x_continuous(limits = c(2015, 2100), breaks = seq(2015, 2100, 20), expand = c(0, 0)) +
  scale_color_manual(
  ...
  theme(
    ...
    axis.title.x = element_text(size = 18, margin = margin(r = 20), hjust=.47),
