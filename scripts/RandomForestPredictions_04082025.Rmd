...
```{r}
library(dplyr)
r_files_historic <- list.files(dir, pattern="mosaic-hist", full.names = TRUE, recursive = TRUE)
r_files_intermediate <- list.files(dir, pattern="mosaic-ncar", full.names = TRUE, recursive = TRUE)
r_files_extreme <- list.files(dir, pattern="mosaic-gfdl", full.names = TRUE, recursive = TRUE)
for (r in seq_along(list_of_rasters)){
  print(terra::sources(list_of_rasters[[r]]))
  print(names(list_of_rasters[[r]]))
}
...
read_raster <- function (file) {
  ...
  if (length(names(r))>41){
    r <- r[, c(1:41)]
  } else {
    print("raster has expected number of bands before adding x and y")
  }
  r$dep2_permafrost <- as.integer(r$dep2_permafrost)#dbl to int
...
Define a function to perform predictions. Using gsub and string split to grab unique identifiers from file names so I can store them in a dataframe. This takes the final fit from the random forest model in the training fitting function later.
...
train_model <- function(df, species, response="Carbon_gm2") {
  ...
  split_data <- initial_split(training_df_one_spp %>% select(all_of(response), all_of(predictors)), prop = 0.90, strata = all_of(response))
  ...
  models <- list(rf=m_rf,ranger=m_ranger,xgb=m_xgb)
  ...
  grids <- list(rf_grid=top3_grid, ranger_grid=top3_grid, xgb_grid=top3_grid)
  ...
  tuned_results <- workflow() %>% add_recipe(recipe) %>% add_model(models[[1]]) %>% 
    tune_grid(resamples = pv_folds, grid = grid[[1]], metrics = metric_set(rmse, rsq, mae))
  
  best_params <- select_best(tuned_results, metric="rmse")
  wflow <- finalize_workflow(workflow() %>% add_recipe(recipe) %>% add_model(model), best_params)
  ...
  #   return(list(fit=final_fit,RMSE=mean_rmse,RSQ=mean_rsq))
  # })
  
  #generate predictions (using the regular RF model fit)
  predictions_historic <- map(r_files_historic, function(file){
    predict_from_model(file, results[[1]], species)})
  predictions_intermediate <- map(r_files_intermediate, function(file){
    predict_from_model(file, results[[1]], species)})
  predictions_extreme <- map(r_files_extreme, function(file){
    predict_from_model(file, results[[1]], species)})
  
  all_predictions <- list(predictions_historic, predictions_intermediate, predictions_extreme)
  all_predictions_df <- bind_rows(all_predictions)
  
  
  return(list(train_data=train_data, test_data=test_data, model_results=results, predictions=all_predictions_df))
}

#run the models
models <- lapply(species, function(spp){train_model(training_df,species=spp)})
```
