...
train_model <- function(df, species, response="Carbon_gm2") {
  ...
  top3_grid <- expand.grid(mtry = seq(5, 20, 5), trees = seq(300,500,25), min_n = seq(1, 12, 2))
...
#write.csv(predictions, "data/PredictedCarbon_RasterValues.csv", row.names = FALSE)
```
...
Graph carbon through time from the rasters we made predictions on. I switch to python here for the aesthetics from matplotlib

```{python}
import pandas as pd
import matplotlib.pyplot as plt

predicted_data = pd.read_csv(validations, "data/PredictedCarbon_RasterValues.csv", row.names = FALSE)

fig, axes = plt.subplots(nrows=1, ncols=4, figsize=(6, 6))
colors = plt.cm.cividis_r(indices['ClimateScenario'])
fig.subtitle("Predicted carbon through time", fontsize=14, ha='center')
plt.tight_layout()
plt.show()
```