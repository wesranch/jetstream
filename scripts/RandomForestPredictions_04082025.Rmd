...
training_df <- read.csv("data/analysis/RandomForest_TrainingDATA_04242025.csv") %>% select(-PSP) %>% select(-starts_with("tsf_")) #%>% select(-dep2_permafrost, -pr, -tas)
...
nmds_rf <- function(raw_model, filtered_train_df, species) {
  ...
  #classic mds
  mds_coords <- cmdscale(1 - prox, k = 2)
...
train_model <- function(df, species, response="Carbon_gm2") {
  ...
  rf_grid <- expand.grid(mtry = seq(5, 20, 5), trees = seq(200,800,400), min_n = seq(1, 12, 3))
  ...
  mds_df <- nmds_rf(raw_model, training_df_one_spp, species)
  
  #plot for one species
  mdsplot <- ggplot(mds_df, aes(x = Dim1, y = Dim2, color = Carbon_gm2)) +
...
proximity_data <- do.call(rbind, lapply(model_output[1:4], function(x) x$mds_data))
...
write.csv(proximity_data, "data/analysis/MDS_Data_RS.csv", row.names = FALSE)
