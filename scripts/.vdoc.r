...
training_df <- read.csv("data/RandomForest_TrainingDATA_04082025.csv") %>% select(-PSP) %>% select(-starts_with("tsf_")) #%>% select(-dep2_permafrost, -pr, -tas)
...
regularPoints <- spatSample(ref, size=200000, method = "random", as.points = TRUE, ext = bounds, na.rm=TRUE)
...
plot(sample(regularPoints, 1000), add = TRUE, pch = 15, col = "orange")#just plotting 1000
