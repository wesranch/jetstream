...
dir <- "C:/Users/wesr/Downloads/drive-download-20250410T215354Z-001/"
training_files <- list.files(dir, pattern = "all", full.names = T)
training_df <- map_dfr(training_files, ~{
  df <- readr::read_csv(.x, show_col_types = FALSE)
  if (ncol(df) == 49) df else NULL
})
```
...
species <- c("AKBirch", "QAspen", "WhiteSpruce","BlackSpruce")

train_model <- function(df, species, response="BA") {
  print(paste0("running model for: ", species))
  
  #filter to one species
  training_df_one_spp <- training_df %>% filter(Species == species) %>% 
    select(-`system:index`, -.geo, -Year, -PSP, -tsf) %>% drop_na()
  predictors <- setdiff(names(training_df_one_spp), c("Species", response))
  ...
  rf_grid <- expand.grid(mtry = seq(5, 20, 5), trees = seq(300,1000,100), min_n = seq(1, 12, 3))
